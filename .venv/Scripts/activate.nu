...
export-env {
    ...
    let virtual_env = r#'D:\vs\homeworks\project1_Shevchenko_Sergey\.venv'#
    let bin = r#'Scripts'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'project1-shevchenko-sergey-py3.12'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'project1-shevchenko-sergey-py3.12'#
    }
